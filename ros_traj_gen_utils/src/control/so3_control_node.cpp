...
//
// This node owns its own OffboardControlMode(body_rate=true) heartbeat and
// enable/disable services, deliberately mirroring the pattern already
// established in traj_gen_test/offboard_bridge.py for its own
// position-mode heartbeat -- the two are NOT meant to stream to PX4 at the
// same time (PX4 only accepts one active OffboardControlMode declaration),
// so wiring this into the actual flight sequence (in place of, not
// alongside, offboard_bridge.py's TrajectorySetpoint relay) is a separate
// integration step.
...
public:
    So3ControlNode() : rclcpp::Node("so3_control_node") {
        ...
        enable_srv_ = create_service<std_srvs::srv::Trigger>(
            device_ + "/enable_rate_control",
            [this](const std::shared_ptr<std_srvs::srv::Trigger::Request>,
                   std::shared_ptr<std_srvs::srv::Trigger::Response> response){
                streaming_ = true;
                response->success = true;
                RCLCPP_INFO(get_logger(), "Rate-control streaming enabled.");
            });
        disable_srv_ = create_service<std_srvs::srv::Trigger>(
            device_ + "/disable_rate_control",
            [this](const std::shared_ptr<std_srvs::srv::Trigger::Request>,
                   std::shared_ptr<std_srvs::srv::Trigger::Response> response){
                streaming_ = false;
                response->success = true;
                RCLCPP_INFO(get_logger(), "Rate-control streaming disabled.");
            });
...
private:
    ...
    void tick(){
        if(!streaming_){
            return; // stream is off until enable_rate_control is called
        }
